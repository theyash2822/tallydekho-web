    ...
    win.document.write(`
      ...
      <div class="header">
        <div>
          ...
          ${data?.voucher?.date ? `<div style="color:#787774;font-size:11px;margin-top:4px">Date: ${data.voucher.date}</div>` : ''}
        </div>
      ...
      <table>
        ...
        <tbody>
          ${(data?.items || []).map(item => `
            <tr>
              <td>${item.ledger_name || '—'}${item.item_name ? ` <span style="color:#787774">(${item.item_name})</span>` : ''}</td>
