...
const kpis = {
  TDS:[{t:'Deducted',v:'₹42,000',a:'#37352F'},{t:'Remitted',v:'₹29,250',a:'#0F7B6C'},{t:'Pending Pay',v:'₹12,750',a:'#F59E0B'},{t:'Late Fee',v:'—',a:'#787774'}],
  TCS:[{t:'Collected',v:'₹18,200',a:'#37352F'},{t:'Remitted',v:'₹18,200',a:'#0F7B6C'},{t:'Pending Pay',v:'—',a:'#F59E0B'},{t:'Late Fee',v:'—',a:'#787774'}],
};
...
const tdsCols = [
  ...
  {key:'section',label:'Section',render:v=><span className="font-mono text-xs bg-[#F7F7F5] px-1.5 py-0.5 rounded">{v}</span>},
...
export default function OtherTaxes() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-xl font-semibold text-[#37352F] tracking-tight">Other Taxes</h1><p className="text-sm text-[#787774] mt-0.5">FY 2025-26 · July 2025</p></div>
      <div className="bg-white border border-[#D3D1CB] rounded-xl">
        <div className="flex border-b border-[#D3D1CB] px-1 pt-1 overflow-x-auto">
          {TAX_TABS.map(t=>(
            <button key={t} onClick={()=>setTaxTab(t)} className={`px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-colors rounded-t-lg mr-1 ${taxTab===t?'text-[#37352F] bg-[#EFEFEF]':'text-[#787774] hover:text-[#37352F] hover:bg-[#F7F7F5]'}`}>{t}</button>
          ))}
        </div>
              ...
              <div className="space-y-2">
                {lateChallans.map((c,i)=>(
                  <div key={i} className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-semibold text-[#37352F]">{fmt(c.amount)}</span>
                    <span className="text-xs text-[#EB5757] font-medium">{c.daysLate} days overdue</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {taxTab==='TDS'?(
            <>
              <div className="flex justify-between items-center">
                ...
                <button className="px-3 py-1.5 border border-[#D3D1CB] text-[#787774] text-xs rounded-lg hover:bg-[#F7F7F5]">Export CSV</button>
              </div>
        ...
        {drawer&&(
          <div className="space-y-3 text-sm">
            {[['Date',drawer.date],['Voucher',drawer.voucher],['Party',drawer.party],['Section',drawer.section],['Amount',fmt(drawer.amount)],['Challan',drawer.challan||'Pending'],['Status',drawer.status]].map(([l,v])=>(
              <div key={l} className="flex justify-between py-2 border-b border-[#F7F7F5]">
                <span className="text-[#787774]">{l}</span><span className="font-medium text-[#37352F]">{v}</span>
              </div>
            ))}
            <button className="w-full py-2.5 mt-2 rounded-lg text-sm font-medium text-white" style={{background:'#37352F'}}>Mark as Remitted</button>
          </div>
        )}
      </Drawer>
    </div>
