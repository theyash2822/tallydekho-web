...
export default function EWayBill() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-xl font-semibold text-[#1C2B3A] tracking-tight">E-Way Bill</h1><p className="text-sm text-[#787774] mt-0.5">July 2025</p></div>
      <div className="grid grid-cols-4 gap-3">
        <KPICard title="Total EWBs"    value="7" sub="July 2025" icon={Truck}         accent="#1C2B3A" />
        <KPICard title="Active"         value="4"               icon={CheckCircle}  accent="#2D7D46" />
        <KPICard title="Expiring Today" value="1"               icon={AlertTriangle} accent="#D97706" />
        ...
        <div className="col-span-2 bg-white border border-[#D4D3CE] rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={ewbPerDay} barSize={16}>
              ...
              <Bar dataKey="count" name="EWBs" fill="#1C2B3A" radius={[4,4,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white border border-[#D4D3CE] rounded-xl p-5">
          ...
          <div className="space-y-2">
            {ewayBills.slice(0,4).map(e => (
              <div key={e.id} className="flex items-start justify-between cursor-pointer hover:bg-[#F5F4EF] p-2 rounded-lg" onClick={() => setDrawer(e)}>
                <div><p className="text-sm font-medium text-[#1C2B3A]">{e.party.split(' ')[0]}</p><p className="text-xs text-[#787774]">{e.ewbNo.slice(-6)} · {e.date}</p></div>
      ...
      <div className="bg-white border border-[#D4D3CE] rounded-xl">
        <div className="flex justify-between items-center px-5 py-4 border-b border-[#D4D3CE]">
          ...
          <div className="flex gap-2">{['PDF','CSV'].map(b => <button key={b} className="px-3 py-1.5 border border-[#D4D3CE] text-[#787774] text-xs rounded-lg hover:bg-[#F5F4EF]">Export {b}</button>)}</div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {ewayBills.map((e, i) => (
                <tr key={e.id} className="border-b border-[#F5F4EF] hover:bg-[#F5F4EF] cursor-pointer" onClick={() => setDrawer(e)}>
                  <td className="px-4 py-3 font-mono text-xs text-[#1C2B3A] font-semibold">{e.ewbNo}</td>
                  ...
                  <td className="px-4 py-3" onClick={ev => ev.stopPropagation()}>
                    <div className="flex gap-1">
                      {e.status === 'Active' && <button className="px-2 py-1 text-xs bg-[#ECEEEF] text-[#1C2B3A] rounded-md hover:bg-[#D4D3CE]">Extend</button>}
                      {e.status !== 'Cancelled' && <button className="px-2 py-1 text-xs bg-[#FDECEA] text-[#C0392B] rounded-md hover:bg-[#FAD8D5]">Cancel</button>}
                      <button className="px-2 py-1 text-xs bg-[#F5F4EF] text-[#787774] rounded-md hover:bg-[#ECEEEF]">PDF</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {drawer && (
          <div className="space-y-5">
            <div className="flex justify-between items-start">
              <div><p className="font-semibold text-[#1C2B3A]">{drawer.party}</p><p className="font-mono text-xs text-[#787774]">{drawer.ewbNo}</p></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[['Date',drawer.date],['From',drawer.from],['To',drawer.to],['Mode',drawer.mode],['Vehicle',drawer.vehicle],['Amount',fmt(drawer.amount)],['Valid Till',drawer.validity]].map(([l,v]) => (
                <div key={l} className="p-3 bg-[#F5F4EF] rounded-lg border border-[#D4D3CE]">
            ...
            <div className="flex gap-2">
              ...
              <button className="px-4 py-2 rounded-lg text-sm font-medium text-[#787774] border border-[#D4D3CE] hover:bg-[#F5F4EF] transition-colors">Share</button>
            </div>
          </div>
        )}
      </Drawer>
