...
const ALL_NOTIFICATIONS = [
  { id:1, title:'GSTR-3B Filing Due',        desc:'GSTR-3B for July 2025 is due in 3 days. 2 mismatches need resolution before filing.', time:'5 mins ago',  tag:'Urgent',   category:'Compliance', color:'#F43F5E', icon:AlertTriangle, read:false },
  { id:2, title:'Tally Sync Complete',        desc:'Your Tally Prime data has been synced successfully. 48 new vouchers, 12 ledgers updated.', time:'32 mins ago', tag:'Info',     category:'Sync',       color:'#059669', icon:CheckCircle,  read:false },
  ...
  { id:4, title:'Payment Received',           desc:'₹9,80,000 received from Reliance Retail Ltd. via NEFT. Invoice SI-2025-0782 marked paid.', time:'2 hrs ago',  tag:'Success',  category:'Transactions',color:'#059669', icon:TrendingUp,   read:true  },
  ...
  { id:8, title:'GSTR-2A Mismatch Detected', desc:'2 invoice mismatches found in GSTR-2A. Bharat Chemicals (GSTIN Mismatch), Sunrise Electricals (Invoice Not Found).', time:'Yesterday', tag:'Urgent',   category:'Compliance', color:'#F43F5E', icon:AlertTriangle, read:true  },
...
const tagColors = { Urgent: 'bg-rose-50 text-rose-600 border border-rose-200', Warning: 'bg-amber-50 text-amber-600 border border-amber-200', Success: 'bg-[#ECFDF5] text-[#059669] border border-[#6EE7B7]', Info: 'bg-blue-50 text-blue-600 border border-blue-200' };
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#1A1A1A] tracking-tight flex items-center gap-2">
            Notifications
            {unreadCount > 0 && (
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold text-white" style={{ background: '#F43F5E' }}>{unreadCount}</span>
            )}
          </h1>
          <p className="text-sm text-[#787774] mt-0.5">{filtered.length} notifications</p>
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllRead} className="text-xs text-[#059669] hover:underline font-medium">Mark all as read</button>
        )}
      </div>
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-colors flex-shrink-0 ${tab === t ? 'text-white' : 'text-[#787774] bg-white border border-[#E8E7E3] hover:border-[#059669]'}`}
            style={tab === t ? { background: '#059669' } : {}}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="bg-white border border-[#E8E7E3] rounded-2xl py-16 text-center">
            <Bell size={36} className="mx-auto mb-3 text-[#AEACA8]" />
            <p className="text-sm font-medium text-[#787774]">No notifications</p>
            <p className="text-xs text-[#AEACA8] mt-1">You're all caught up!</p>
          </div>
        ) : filtered.map(n => {
          const Icon = n.icon;
          return (
            <div key={n.id} onClick={() => markRead(n.id)}
              className={`flex gap-4 p-4 rounded-2xl border cursor-pointer transition-all hover:shadow-notion ${n.read ? 'bg-white border-[#E8E7E3]' : 'bg-[#FAFFFE] border-[#6EE7B7]'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  {!n.read && <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 ml-1" style={{ background: '#059669' }} />}
                </div>
              </div>
            </div>
          );
