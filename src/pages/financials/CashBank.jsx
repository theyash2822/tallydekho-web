  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <KPICard title="Net Cash Flow"   value={loading ? '—' : fmtL(summary.netCash)}        icon={RefreshCw}    accent="#3F5263" />
      </div>
      ...
      <div className="bg-white border border-[#D9DCE0] rounded-xl">
        ...
        <div className="p-4">
          {loading ? (
            ...
            txns.length === 0
              ? <div className="h-40 flex items-center justify-center text-xs text-[#9CA3AF]">{isPaired ? 'No transactions found for this period' : 'Pair desktop app to see transactions'}</div>
              : <Table columns={txnCols} data={txns} />
          ) : (
            banks.length === 0
              ? <div className="h-40 flex items-center justify-center text-xs text-[#9CA3AF]">{isPaired ? 'No bank accounts found' : 'Pair desktop app to see bank accounts'}</div>
